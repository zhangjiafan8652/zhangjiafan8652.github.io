...
else e(jQuery)}(function(e){var n=/\+/g
function i(e){return u.raw?e:encodeURIComponent(e)}function o(e){return u.raw?e:decodeURIComponent(e)}function r(e){return i(u.json?JSON.stringify(e):String(e))}function t(e){if(0===e.indexOf('"'))e=e.slice(1,-1).replace(/\\"/g,'"').replace(/\\\\/g,"\\")
try{e=decodeURIComponent(e.replace(n," "))
return u.json?JSON.parse(e):e}catch(i){}}function c(n,i){var o=u.raw?n:t(n)
return e.isFunction(i)?i(o):o}var u=e.cookie=function(n,t,f){if(void 0!==t&&!e.isFunction(t)){f=e.extend({},u.defaults,f)
if("number"===typeof f.expires){var a=f.expires,s=f.expires=new Date
s.setTime(+s+864e5*a)}return document.cookie=[i(n),"=",r(t),f.expires?"; expires="+f.expires.toUTCString():"",f.path?"; path="+f.path:"",f.domain?"; domain="+f.domain:"",f.secure?"; secure":""].join("")}var d=n?void 0:{}
var p=document.cookie?document.cookie.split("; "):[]
for(var v=0,m=p.length;v<m;v++){var l=p[v].split("=")
var x=o(l.shift())
var k=l.join("=")
if(n&&n===x){d=c(k,t)
break}if(!n&&void 0!==(k=c(k)))d[x]=k}return d}
u.defaults={}
e.removeCookie=function(n,i){if(void 0===e.cookie(n))return false
e.cookie(n,"",e.extend({},i,{expires:-1}))
return!e.cookie(n)}})
...
/**
 Created by codeenxi(352287526@qq.com) on 2014/11/28. */

var Api = window.Api={};
var popDialog = window.popDialog = {};//global dialog mask
var toast = window.toast = {};
(function ($) {
    popDialog.mask = {
        show: function (content,isloading) {
            if(content){
                var maskDiv = '<div class="page-info-mask"><div class="page-log-info">'+content+'</div></div>';
            }else if(isloading){
                var maskDiv = '<div class="page-loading"></div>';
            }else{
                var maskDiv = '<div class="page-info-mask"></div>';
            }
            $('.page-info-mask').remove();
            $('body').append(maskDiv);
            $('.page-info-mask').height($('body').height());
            $('.page-loading').height($('body').height());
            return true;
        },
        hide: function (isloading) {
            if(isloading){
                $('.page-loading').remove();
            }else{
                $('.page-info-mask').remove();
    ...
    popDialog.popInfo = {
        show:function (content) {
            if(content){
                //var popDiv = '<div class="page-pop-info"><div class="page-pop-info">'+content+'</div></div>';
                var popDiv = content.jquery ? content : ''
                    +'<div class="page-pop-info">'
                        +'<div class="page-pop-info-content">'
                            +'<p>十分抱歉，'+content+'号已被预订，建议您更改出行日期。</p>'
                            +'<p class="regard">给你造成的不便，再次深表歉意</p>'
                        +'</div>'
                        +'<a href="javascript:void(0);">知道了</a>'
                    +'</div>'
                $('.page-pop-info').remove();
                $('body').append(popDiv);

                $('.page-pop-info').on('click',function (e) {
                    e = e||window.event;
                    var currentElement = e.target;
                    if($(currentElement).text()=='知道了'){
                        $('.page-pop-info').off('click');
                        $('.page-pop-info').remove();
                    }
                })
            }
    ...
    toast = {
        ...
        show:function (content) {
            if(content){
                var toastDiv = '<div class="page-info-toast">'+content+'</div>';
            }else{
                var toastDiv = '<div class="page-info-toast"></div>';
            }
            $('.page-info-toast').remove();
    ...
    * 所有Api配置
    * */
     var base_url = 'http://api.zouzhe.com';
    //var base_url = location.host == 'www.zouzhe.com' ? 'http://api.zouzhe.com' : 'http://api.66hao.com';
    //var base_url = 'http://192.168.1.188';  
    //var test_url = base_url + '/wechat/api/route/list';
    Api = {
        /*
            index module

        */
        getBannerInfo: base_url + '/wechat/api/news/list',//首页banner信息
        getSpecialRoute: base_url + '/wechat/api/driver/search/result',//首页精选路线推荐
        getMoreRoute: base_url + '/wechat/api/driver/search/result',//获取更多路线
        /*
            user module
        */
        login: base_url + "/wechat/api/traveller/login/v2",//登录
        logout: base_url + '/wechat/api/traveller/logout',//用户登出
        getUserInfo: base_url + '/wechat/api/traveller/detail/v2',//用户中心获取用户信息
        getValidCode: base_url + '/wechat/api/traveller/valid_code',//获取用户验证码
        check_expire: base_url + '/wechat/api/traveller/check_expire',//检查是否过期
        checkMobile: base_url +  '/wechat/api/traveller/check_mobile',//检查手机是否注册过
        registe: base_url + '/wechat/api/traveller/register/v2',//用户注册
        saveUserInfo: base_url + '/wechat/api/traveller/save/v2',//保存用户资料
        saveUserHeadImg: base_url + '/wechat/api/traveller/save_by_image',//保存用户头像
        resetPassword: base_url + '/wechat/api/traveller/reset_password',//重置密码
        checkExpire: base_url + '/wechat/api/traveller/check_expire',//检查用户是否过期

        /*
            routes module
        */
        routeList: base_url + '/wechat/api/route/list',//线路列表
        routeDetail: base_url + '/wechat/api/route/detail',//线路详情
        /*
            driver module
        */
        //searchDriver: base_url + '/wechat/api/driver/search/result',//司机查询
        driverList: base_url + '/wechat/api/driver/list',//司机列表
        driverDetail: base_url + '/wechat/api/driver/detail/v2',//司机详情
        driverSearch: base_url + '/wechat/api/driver/search',//司机查询
        searchPlace: base_url + '/wechat/api/driver/place',
        specialRouteDriverSearch: base_url + '/wechat/api/driver/group',//首页出发地司机分组
        specialDriver: base_url + '/wechat/api/driver/list', //首页推荐司机列表
        driverRoute:base_url + '/wechat/api/driver/route',//司机详情页，司机路线
        checkDriverSchedule:base_url + '/wechat/api/driver/schedule_check',//检查档期

        passengerComment:base_url +'/wechat/api/driver/comments',
        //passengerComment:'http://192.168.1.200:8080' +'/wechat/api/driver/comments',

        /*
            news module
        */
        getNewsDetail: base_url + '/wechat/api/news/detail',
        applyActivity: base_url + '/wechat/api/news/apply',
        checkActivityStatus: base_url + '/wechat/api/news/check_apply',

        /*
            order module
        */
        getOrderList: base_url + '/wechat/api/order/list/v2',//用户中心，用户订单列表
        orderInit: base_url + '/wechat/api/order/init',
        saveOrder: base_url + '/wechat/api/order/save/v2',//保存订单
        getOrderDetail:base_url + '/wechat/api/order/detail/v2',
        checkCoupon:base_url+ '/wechat/api/order/check_coupon/v2', //check coupon
        payOrder:base_url + '/wechat/api/order/pay',
        getCouponList:base_url + '/wechat/api/order/coupon_list'

    };
    var loginfo = document.createElement("div");
    document.body.appendChild(loginfo);
    $(loginfo).on("popup.show", function () {
        //console.log('服务器错误');
    });
    var loading = $('.loading');
    function httpFail(jqXHR, options) {
        $(loginfo).trigger('popup.show');
    }
    function redircetToLogin(){
...
/*                  dataType: 'json',
                    ...
                    data: JSON.stringify(options && options.data),*/
                    data:options && options.data
                }).then(function(data, textStatus, jqXHR) {
                    data = JSON.parse(data);
                    //(data.status == 0) && httpFail();
                    (data.status == -1)&&redircetToLogin();//未登录则强制跳转到首页，在这里暂时屏蔽
                    return data.page && data.page.result || data;
                }).fail(function(jqXHR, textStatus, errorThrown) {
                    httpFail(jqXHR, options);
                }).always(function() {
    ...
    //document.getElementsByTagName('a')[0].style.display="none";//hide baidu tongji logo
    
})(jQuery);

//分页方法(赖皮版)

function pagination (parentBox,pageData,callback,searchData) {
    if(!parentBox){
        return;
    }
    parentBox.empty();
    var totalPage = pageData.total_page;//total page
    var currentPage = pageData.current_page||pageData.current;//current page
    if(totalPage==1){
        return;
    }
    if(totalPage>0&&parentBox){
        var pageHtml = [];
        for(var i=1;i<=totalPage;i++){//此处应该根据当前页面，取前后页码，在此处我偷懒了
            if(i==currentPage){
                pageHtml.push('<li class="current-page">'+i+'</li>');
            }else{
                pageHtml.push('<li>'+i+'</li>');
            }   
        }
        pageHtml.join('');
        parentBox.append(pageHtml);
        parentBox.off('click');
        parentBox.on('click',function (e) {
            e = e||window.event;
            var currentElement = e.target;
            if(currentElement.tagName =='LI'){
                var pageNumber = $(currentElement).text();
                //callback&&callback(searchData, pageNumber);
                var args = [searchData, pageNumber];
...
//render route item
//生成路线item结构
function renderRouteContent(data){
    /*
    * data:
        name 线路名称
        title 标题
        ...
        departure.name 出发地
        price 价格
        //extra info from static variable
        url 路线详情页链接
        route_type: 1--common  2--activity  3--discount
        price 价格
        discount_price 优惠价格
    */
    var type = data.route_type || 2;
    var price = data.price;
    var url = data.url || 'news.html?newsId=54d204bfc508d77c267f4154';
    var itemClass = type == 1 ? 'common-type' : type == 2 ? 'activity-type' : 'discount-type';
    // var str = type == 1 ? ('<span>￥' + price + '起</span>') : 
    var str = type == 1 ? ('<span>' + price + '</span>') : 
              // type == 2 ? ('<span>立即报名</span><span>整车￥' + price + '起</span>') ://load api
              type == 2 ? ('<span>整车￥' + price + '起</span>') : //fake data
              ('<span>￥' + data.discount_price + '/人</span>' + '<span class="discount-price">原价￥' + price + '</span>');

    // var str = '<span>￥' + data.discount_price + '/人</span>' + '<span class="discount-price unthrougth">原价￥' + price + '</span>'

    return '<div class="route-item ' + itemClass + '">'
                + '<a href="' + url + '">'
                    + '<div class="img-content" style="background-image:url(' + data.pictures[0] + ')"></div>'
                    + '<div class="route-item-info">'
                        + '<div class="route-info-title text-center">' 
                            // + '<span class="name">从' + data.name + '出发</span>' //调用接口时用
                            +'<span class="name">'+ data.name +'</span>'//假数据用
                            + '<span class="title">' + data.title + '</span>'
                        + '</div>'
                        + '<div class="route-extra-info text-center">'
                            + str
                        + '</div>'
                    + '</div>'
                + '</a>'
...
(function(a) {
    ...
    function p(a, b, c) {
        ...
        for (d = 0; e > d; d++)(c && a[d] !== b[d] ||!c && t(a[d]) !== t(b[d])
            ) && g++;
        return g + f
    }
    function q(a) {
    ...
    }, dc = ["months", "monthsShort", "weekdays", "weekdaysShort", "weekdaysMin"];
    ac.length;
...
var headerDatePicker=window.headerDatePicker||{};!function(a){headerDatePicker={start:a("#startDate"),end:a("#endDate"),today:new Date,minDate:1,init:function(){headerDatePicker.endFun(),headerDatePicker.startFun()},showNext:function(){this.end.focus()},startFun:function(){headerDatePicker.start.datepicker({dateFormat:"yy-mm-dd",dayNamesMin:["日","一","二","三","四","五","六"],monthNames:['1<em class="month">月</em>','2<em class="month">月</em>','3<em class="month">月</em>','4<em class="month">月</em>','5<em class="month">月</em>','6<em class="month">月</em>','7<em class="month">月</em>','8<em class="month">月</em>','9<em class="month">月</em>','10<em class="month">月</em>','11<em class="month">月</em>','12<em class="month">月</em>'],altFormat:"yy-mm-dd",yearSuffix:"年",showMonthAfterYear:!0,firstDay:1,showOtherMonths:!0,minDate:0,maxDate:180,onSelect:function(b){a(".departure-list").hide(),a(".destination-list").hide(),headerDatePicker.end.datepicker("refresh"),headerDatePicker.end.datepicker("option","minDate",new Date(moment(b).add("days",headerDatePicker.minDate))),headerDatePicker.end.datepicker("option","maxDate",new Date(moment(b).add("days",180))),setTimeout(function(){headerDatePicker.showNext()},300)}})},endFun:function(){headerDatePicker.end.datepicker("refresh"),headerDatePicker.end.datepicker({dateFormat:"yy-mm-dd",dayNamesMin:["日","一","二","三","四","五","六"],monthNames:['1<em class="month">月</em>','2<em class="month">月</em>','3<em class="month">月</em>','4<em class="month">月</em>','5<em class="month">月</em>','6<em class="month">月</em>','7<em class="month">月</em>','8<em class="month">月</em>','9<em class="month">月</em>','10<em class="month">月</em>','11<em class="month">月</em>','12<em class="month">月</em>'],altFormat:"yy-mm-dd",yearSuffix:"年",showMonthAfterYear:!0,firstDay:1,showOtherMonths:!0,minDate:1,maxDate:180,onSelect:function(){a(".departure-list").hide(),a(".destination-list").hide()}})}},headerDatePicker.init()}(jQuery);
function buildDeparture(a){var b="<dt>热门出发地</dt><dd>"+driverPlaceList.departure.hotplace.join("</dd><dd>")+"</dd>",c="",d=driverPlaceList.departure.province;for(var e in d)c+="<dt>"+e+"</dt><dd>"+d[e].join("</dd><dd>")+"</dd>";var f=b+c;a.append(f)}function buildDestination(a,b){var c=driverPlaceList.destination.hotplace[b],d=[],e="";c&&($.each(c,function(a,b){var c=b.place,e=b.date;d.push("<dd>"+c+'<span class="least-day">行程至少'+e+"天</span></dd>")}),e="<dt>热门目的地</dt>"+d.join(""));var f=[],g="",h=driverPlaceList.destination.cityList[b];h&&($.each(h,function(a,b){var c=b.place,d=b.date;f.push("<dd>"+c+'<span class="least-day">行程至少'+d+"天</span></dd>")}),g=c?"<dt>其它目的地</dt>"+f.join(""):"<dt>目的地</dt>"+f.join(""));var i=e+g||"暂无对应目的地";a.append(i)}function buildVehicle(a){var b=driverPlaceList.vehicles,c=[];$.each(b,function(a,b){c.push("<dd>"+b.vehicle+"</dd>")}),c.join(""),a.append(c)}var driverPlaceList=window.driverPlaceList||{};!function(){$(function(){var a=($(".search-driver a"),$("#ui-datepicker-div"),$(".search-box")),b=$(".departure-input"),c=$(".destination-input"),d=$(".departure-list"),e=$(".destination-list"),f=$(".search-driver-btn"),g=$("#startDate"),h=$("#endDate"),i=$(".search-nav-departure-list"),j=$(".search-nav-vehicle-list");Api.searchPlace({type:"get"}).then(function(a){a.status&&(driverPlaceList=a.result||driverPlaceListData,buildDeparture(d),buildDeparture(i),buildVehicle(j))}).fail(),$("body").on("click",function(c){var f=c.target;"搜索司机"==$(f).text()?(a.show(),b.focus(),popDialog.mask.show()):"page-info-mask"==$(f).attr("class")&&(a.hide(),d.hide(),e.hide(),popDialog.mask.hide())}),b.on("focus",function(){d.show(),e.hide()}),g.on("focus",function(){d.hide(),e.hide()}),h.on("focus",function(){d.hide(),e.hide()}),d.on("click",function(a){var a=a||window.event,f=a.target;"DD"==f.tagName&&(b.val($(f).text()),d.hide(),e.empty(),buildDestination(e,$(f).text()),setTimeout(function(){c.focus()},300))}),c.on("focus",function(){return""==$.trim(b.val())?void b.focus():(e.show(),void d.hide())}),e.on("click",function(a){var a=a||window.event,b=a.target;if("DD"==b.tagName){var d=/\d+/g.exec($(b).text());c.val($(b).text().replace(/\d+/g,"").replace("行程至少天","")),e.hide(),headerDatePicker.minDate=d,$("#startDate").focus()}}),f.on("click",function(){var a=$.trim(b.val()),d=$.trim(c.val()),e=$.trim(g.val()),f=$.trim(h.val());if(a){var i="./driverList.html?departure="+encodeURIComponent(a)+"&returns="+encodeURIComponent(d)+"&depart_date="+e+"&return_date="+f;location.href=i}else b.focus()})})}(jQuery);
var addLoggedOutMask=function(){var a=function(a){a?$("html,body").css({height:$(window).height()+"px",overflow:"hidden"}):$("html,body").removeAttr("style")},b=window.userStatus,c=$(".login"),d=$('<div class="loggedout-mask"></div>'),e=$("body");$.isEmptyObject(b)&&(c.trigger("click"),e.append(d),a(!0),$(window).on("resize",function(){a(!0)}),$(".dialog-close-icon").addClass("hidden"))};